 ...
 * stereo.
 */
public class p1 extends GLJPanel implements GLEventListener, KeyListener{
    ...
    private void corkscrew(int n,GL2 gl2){

        double x,y,z;

        gl2.glColor3f(1, 0, 1);// różowy
        gl2.glBegin(GL2.GL_LINE_STRIP);

        var a=2*Math.PI /(2*n);// kształt spirali

        for(float i=1;i<=1000*n;i++)
        {
    ...
    {
        gl2.glColor3f(1, 0, 0);// czerwony
        gl2.glBegin(GL_TRIANGLE_FAN);

        gl2.glVertex3d(	(float)(Math.cos(j * 2 * Math.PI / n)),(float)(Math.sin(j * 2 * Math.PI / n)), 2f);
        gl2.glVertex3d(	(float)(Math.cos(k * 2 * Math.PI / n)),(float)(Math.sin(k * 2 * Math.PI / n)), 2f);
        gl2.glVertex3d(0, 0, 0);

        gl2.glEnd();
    }
    private void hexagon(int n,GL2 gl2)
    {
